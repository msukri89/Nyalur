<script>
  ...
  function handleIncomingConnection(conn) {
    receiver = receiveFiles(
      ...
      async (result) => {
        ...
        state = 'complete';
        await addTransfer({
          direction: 'received', peerName: offer?.deviceName || 'Unknown',
          files: result.files.map(f => ({ name: f.name || '', size: f.size || 0 })),
          totalSize: result.totalSize, duration: result.duration, status: 'completed'
        });
      }
    );
  }
  ...
  function acceptTransfer() {
    if (receiver) { receiver.accept(); state = 'receiving'; }
  }

  function rejectTransfer() {
    if (receiver) { receiver.reject(); state = 'waiting'; offer = null; receiver = null; }
  }
  ...
  function saveName() {
    ...
    if (trimmed) {
      ...
      peerManager.setDeviceName(trimmed);
    }
    editingName = false;
  }
  ...
  function reset() {
    state = 'waiting'; offer = null; receiver = null; receivedFiles = []; transferResult = null; errorMessage = '';
  ...
  <div class="flex justify-center mb-4">
    {#if editingName}
      <div class="flex items-center gap-2">
        <input type="text" bind:value={nameInput} bind:this={nameInputEl}
          on:keydown={handleNameKey} on:blur={saveName} maxlength="20"
          class="bg-nyalur-surface border border-nyalur-border rounded-lg px-3 py-1.5 text-sm text-center
                 text-nyalur-text focus:outline-none focus:border-nyalur-orange/50 w-44 font-mono" />
  ...
  <div class="flex-1 flex flex-col items-center">
    {#if state === 'waiting'}
      {#if peerState?.status === 'connecting'}
        <div class="flex-1 flex flex-col items-center justify-center">
          <div class="w-[220px] h-[220px] bg-nyalur-surface rounded-2xl animate-pulse mb-6"></div>
      ...
      {:else if peerState?.status === 'connected'}
        <div class="text-center w-full">
          ...
          <div class="mb-3">
            <span class="text-5xl md:text-6xl font-black tracking-[0.4em] text-nyalur-orange font-mono select-all">{spacedCode}</span>
          </div>
          <p class="text-sm text-nyalur-muted/70 mb-5">Berikan kode ini ke pengirim</p>
          <button on:click={copyRoomCode}
            class="inline-flex items-center gap-2 bg-nyalur-surface border border-nyalur-border rounded-xl
                   px-4 py-2.5 hover:border-nyalur-orange/50 transition-all active:scale-95 mb-6">
            {#if copied}
              <svg class="w-4 h-4 text-nyalur-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
              </svg>
              <span class="text-sm text-nyalur-green font-medium">Tersalin!</span>
            {:else}
              ...
              <span class="text-sm text-nyalur-muted font-medium">Salin Kode</span>
            {/if}
          </button>
          {#if qrDataUrl}
            <div class="flex justify-center mb-6">
              <div class="bg-nyalur-surface rounded-2xl p-3 inline-block border border-nyalur-border">
                <img src={qrDataUrl} alt="QR Code room" class="w-[196px] h-[196px] rounded-lg" />
              </div>
            </div>
          {/if}
          <div class="flex items-center gap-2 justify-center mb-2">
            <span class="w-2 h-2 rounded-full bg-nyalur-orange animate-pulse"></span>
            <p class="text-sm text-nyalur-muted">Menunggu koneksi...</p>
          </div>
        </div>
      {:else if peerState?.status === 'error'}
        <div class="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <button on:click={() => dispatch('back')}
            class="px-6 py-2.5 bg-nyalur-surface border border-nyalur-border rounded-xl hover:bg-nyalur-border transition-colors text-sm active:scale-95">Kembali</button>
        </div>
      {/if}
      ...
      <div class="w-full text-center">
        ...
        <div class="flex gap-3 justify-center">
          <button on:click={rejectTransfer}
            class="flex-1 max-w-[140px] py-3 bg-nyalur-surface border border-nyalur-border rounded-xl hover:bg-nyalur-border transition-colors text-sm active:scale-95">Tolak</button>
          <button on:click={acceptTransfer}
            class="flex-1 max-w-[140px] py-3 bg-nyalur-orange text-nyalur-bg font-bold rounded-xl hover:bg-nyalur-orange/90 transition-colors text-sm active:scale-95">Terima</button>
        </div>
      </div>
    ...
    {:else if state === 'receiving'}
      <div class="w-full flex-1 flex flex-col items-center justify-center text-center">
        ...
        <div class="w-full bg-nyalur-surface rounded-full h-1.5 mb-3">
          <div class="bg-nyalur-orange h-1.5 rounded-full transition-all duration-500" style="width: {progress.totalProgress * 100}%"></div>
        </div>
        ...
        {#if progress.speed > 0}
          <p class="text-xs text-nyalur-muted/60 mt-2">~{formatTime((progress.totalSize - progress.totalReceived) / progress.speed)} lagi</p>
        {/if}
      </div>
    ...
    {:else if state === 'complete'}
      <div class="w-full flex-1 flex flex-col items-center justify-center text-center">
        ...
        {#if transferResult}
          <p class="text-sm text-nyalur-muted mb-4">{transferResult.files.length} file ({formatFileSize(transferResult.totalSize)}) dalam {formatTime(transferResult.duration / 1000)}</p>
        {/if}
        <div class="bg-nyalur-surface rounded-xl p-4 mb-6 text-left w-full">
          {#each receivedFiles as file}
            <div class="flex items-center gap-2 py-2">
              ...
              <span class="text-sm truncate flex-1">{file.name}</span>
              <button on:click={() => handleDownload(file)} class="text-xs text-nyalur-orange hover:underline flex-shrink-0 font-medium">Unduh</button>
            </div>
          {/each}
        </div>
    ...
    {:else if state === 'error'}
      <div class="w-full flex-1 flex flex-col items-center justify-center text-center">
        ...
        <button on:click={() => { reset(); dispatch('back'); }}
          class="px-8 py-3 bg-nyalur-surface border border-nyalur-border rounded-xl hover:bg-nyalur-border transition-colors text-sm active:scale-95">Kembali</button>
      </div>
    {/if}
  </div>
</div>
