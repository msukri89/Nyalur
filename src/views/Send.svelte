...
<div class="min-h-screen flex flex-col px-4 py-6 max-w-lg mx-auto">
  <div class="flex items-center gap-3 mb-6">
    ...
    <div class="flex flex-col"><h1 class="text-xl font-bold text-nyalur-green">KIRIM FILE</h1>{#if deviceName}<span class="text-xs text-nyalur-muted/60">{deviceName}</span>{/if}</div>
    {#if peerState?.status === 'connected'}<span class="ml-auto text-xs text-nyalur-green/60 flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-nyalur-green"></span>Online</span>{/if}
  </div>
  ...
  <div class="flex-1 flex flex-col">
    {#if state === 'idle' || state === 'connecting'}
      <label class="block w-full mb-4 cursor-pointer" on:dragover={handleDragOver} on:dragleave={handleDragLeave} on:drop={handleDrop}>
        <div class="border-2 border-dashed rounded-xl p-6 text-center transition-all active:scale-[0.98] {dragOver ? 'border-nyalur-green bg-nyalur-green/10' : 'border-nyalur-border hover:border-nyalur-green/50 hover:bg-nyalur-green/5'}">
          <svg class="w-10 h-10 text-nyalur-muted mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
      ...
      {#if selectedFiles.length > 0}
          ...
          {#each selectedFiles as file, i}
            <div class="bg-nyalur-surface rounded-xl px-3 py-2.5 flex items-center gap-3">
              <div class="w-8 h-8 rounded-lg bg-nyalur-green/15 flex items-center justify-center flex-shrink-0">
                <svg class="w-4 h-4 text-nyalur-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              </div>
              <div class="flex-1 min-w-0"><p class="text-sm truncate">{file.name}</p><p class="text-xs text-nyalur-muted">{formatFileSize(file.size)}</p></div>
              <button on:click={() => removeFile(i)} class="text-nyalur-muted hover:text-nyalur-error transition-colors p-1"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
          {/each}
        ...
        <div class="mb-4">
          ...
          <div class="flex gap-2 items-center">
            <input type="text" value={roomCode} on:input={handleRoomCodeInput} placeholder="XXXX" maxlength="4" class="flex-1 bg-nyalur-surface border border-nyalur-border rounded-xl px-4 py-3.5 text-nyalur-text placeholder-nyalur-muted/30 focus:outline-none focus:border-nyalur-green/50 text-2xl font-mono tracking-[0.3em] text-center uppercase disabled:opacity-50 transition-colors" disabled={state === 'connecting'} />
            <button on:click={startScanner} disabled={state === 'connecting'} class="w-[52px] h-[52px] flex-shrink-0 bg-nyalur-surface border border-nyalur-border rounded-xl flex items-center justify-center hover:border-nyalur-green/50 hover:bg-nyalur-green/5 active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed transition-all" title="Scan QR Code">
              <svg class="w-6 h-6 text-nyalur-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 7V5a2 2 0 012-2h2m0 16H5a2 2 0 01-2-2v-2m16-10V5a2 2 0 00-2-2h-2m0 16h2a2 2 0 002-2v-2" /><rect x="7" y="7" width="4" height="4" rx="0.5" fill="none" /><rect x="13" y="7" width="4" height="4" rx="0.5" fill="none" /><rect x="7" y="13" width="4" height="4" rx="0.5" fill="none" /></svg>
            </button>
          </div>
          <p class="text-xs text-nyalur-muted/40 mt-2">Masukkan 4 karakter kode room, atau scan QR dari penerima</p>
        </div>
        <button on:click={connect} disabled={!canSend} class="w-full py-3.5 bg-nyalur-green text-nyalur-bg font-bold rounded-xl hover:bg-nyalur-green/90 active:scale-[0.98] disabled:opacity-30 disabled:cursor-not-allowed transition-all text-sm flex items-center justify-center gap-2">
          {#if state === 'connecting'}<svg class="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" class="opacity-25" /><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" class="opacity-75" /></svg><span>Menghubungkan...</span>{:else}<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7 11l5-5m0 0l5 5m-5-5v12" /></svg><span>Kirim</span>{/if}
        </button>
      {/if}
      ...
      <div class="flex-1 flex flex-col items-center">
        <div class="w-full mb-4">
          <div class="flex items-center justify-between mb-3">
            ...
            <button on:click={stopScanner} class="w-8 h-8 rounded-lg bg-nyalur-surface flex items-center justify-center hover:bg-nyalur-border active:scale-95 transition-colors"><svg class="w-4 h-4 text-nyalur-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
          </div>
    ...
    {:else if state === 'transferring'}
      <div class="flex-1 flex flex-col items-center justify-center text-center">
        <div class="w-28 h-28 rounded-full flex items-center justify-center mx-auto mb-6 relative">
          <svg class="absolute w-28 h-28 -rotate-90" viewBox="0 0 96 96"><circle cx="48" cy="48" r="42" fill="none" stroke="#334155" stroke-width="4" /><circle cx="48" cy="48" r="42" fill="none" stroke="#39FF14" stroke-width="4" stroke-dasharray="263.9" stroke-dashoffset={progressOffset} stroke-linecap="round" class="transition-all duration-500" /></svg>
        ...
        <div class="w-full bg-nyalur-surface rounded-full h-1.5 mb-3"><div class="bg-nyalur-green h-1.5 rounded-full transition-all duration-500" style="width: {progress.totalProgress * 100}%"></div></div>
        <div class="flex justify-between text-xs text-nyalur-muted w-full"><span>{formatFileSize(progress.totalSent)} / {formatFileSize(progress.totalSize)}</span><span>{formatSpeed(progress.speed)}</span></div>
        {#if progress.speed > 0}<p class="text-xs text-nyalur-muted/60 mt-2">~{formatTime((progress.totalSize - progress.totalSent) / progress.speed)} lagi</p>{/if}
      </div>

    {:else if state === 'complete'}
      <div class="flex-1 flex flex-col items-center justify-center text-center">
        <div class="w-20 h-20 rounded-full bg-nyalur-green/15 flex items-center justify-center mx-auto mb-6"><svg class="w-10 h-10 text-nyalur-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg></div>
        <h2 class="text-2xl font-bold text-nyalur-green mb-2">Berhasil!</h2>
        {#if result}
          <p class="text-sm text-nyalur-muted mb-1">{result.files.length} file ({formatFileSize(result.totalSize)}) terkirim dalam {formatTime(result.duration / 1000)}</p>
          <p class="text-xs text-nyalur-muted/50">Kecepatan rata-rata: {formatSpeed(result.avgSpeed || 0)}</p>
        {/if}
